    ...
    public function updateAppointmentStatus(Appointment $appointment, array $data, $user)
    {
        ...
        if ($conversation && isset($data['status'])) {
            ...
            if ($data['status'] === 'scheduled') {
                ...
                $locationStr = $appointment->location ?? 'TBD';
                $meetingLinkStr = isset($data['meeting_link']) && $data['meeting_link'] ? "<br><br><a href='{$data['meeting_link']}' target='_blank' style='display: inline-flex; align-items: center; gap: 8px; margin-top: 8px; padding: 8px 16px; background-color: #4f46e5; color: white; font-weight: bold; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); text-decoration: none; font-size: 14px;'>🎥 Join Online Meeting</a>" : '';

                Message::create([
                    'uuid' => (string) Str::uuid(),
                    'conversation_id' => $conversation->id,
                    'sender_id' => $doctorId,
                    'message' => "Appointment scheduled on <b>{$dateStr}</b> at <b>{$locationStr}</b>.{$meetingLinkStr}\n[APPOINTMENT_SCHEDULED:{$appointment->uuid}]",
                ]);
